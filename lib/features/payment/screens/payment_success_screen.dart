...
class _CollectItemScreenState extends ConsumerState<CollectItemScreen> {
  ...
  Future<void> _openLocker({
    ...
    try {
      ...
      setState(() {
        _unlockInfo = info;
        _stage = info.boxNumbers.length == 1
            ? 'Opening Box ${info.boxNumbers.first}…'
            : 'Opening boxes…';
      });
      ...
      final result = await engine.unlockOpen(
        ...
        onStage: (stage) {
          ...
          setState(() {
            _stage = switch (stage) {
              'scan' => 'Looking for locker…',
              'connect' => 'Connecting…',
              'connected' => 'Connected',
              'open' => info.boxNumbers.length == 1
                  ? 'Opening Box ${info.boxNumbers.first}…'
                  : 'Opening…',
              'success' => 'Opened',
              _ => _stage,
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              backendOk ? 'Locker opened' : 'Locker opened',
            ),
          ),
        );
      }
  ...
  static String _friendlyCollectError(Object e) {
    ...
    if (lower.contains('write failed') ||
      ...
      return 'Couldn\'t open the locker. Try again.';
    }
